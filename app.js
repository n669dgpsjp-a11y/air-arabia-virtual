...
document.querySelector('#destinationGrid').innerHTML = destinations.map((d,i)=>`<article class="destination-card"><div class="destination-art art-${i+1}"><span>${d[0]}</span><b>✈</b></div><div class="destination-body"><div><small>${d[2]}</small><h3>${d[1]}</h3></div><div class="fare"><small>From</small><strong>0 R$</strong></div></div></article>`).join('');
